...
EXPERIMENT_NAME = "meal_forecasting_no_ref"

@register_experiment(EXPERIMENT_NAME)
class MealForecastingNoRefExperiment(CGMacrosExperiment):
    """
    Experiment for forecasting CGM values after a meal event.
    The agent is given a 1-hour CGM window before a randomly chosen meal, and is asked to forecast CGM for the next 30 minutes after the meal starts.
    """
    ...
    async def run_agent(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Save agent input: 1-hour CGM before meal
        ...
        A meal event occurred at {data["meal_time"]}.
