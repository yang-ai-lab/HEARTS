...
@register_experiment("non_meal_imputation_hr")
class NonMealImputationHRExperiment(CGMacrosExperiment):
    """
    ...
    def __init__(
        self,
        task: str = "non_meal_imputation_hr",
        num_test: int = 50,
        logs_dir=None,
        agent=None,
        data_dir=None,
    ...
    async def run_agent(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        agent_input_df = data["window_df"][
            [self.TIMESTAMP_COLUMN, self.CGM_COLUMN, self.HEART_RATE_COLUMN]
        ]
        ...
        prompt = dedent(f"""\
        The CGM (continuous glucose monitoring) and heart rate (HR, bpm) data for this subject is provided in 'input/{file_name}.csv'. There are three columns: Timestamp (datetime), 'Libre GL' (CGM, mg/dL), and 'HR' (heart rate, beats per minute).
        In this {self.window_hours}-hour window, a {self.mask_minutes}-min segment of CGM readings is corrupted (set to 0), start from {mask_start} and end at {mask_end}. Please impute the missing CGM values in this segment using the heart rate and remaining CGM data. Then save your imputed values in order as a list in json file at `output/solution.json`. When finished, put your methodology to solve this task as your answer.
        """)
        ...
        solution, fail_reason = self.parse_output(query_id)
    ...
    def parse_output(self, query_id: str) -> tuple[List[Any], Any]:
        """Parse the output file for the given query_id."""
        try:
            with open(
            ...
            assert len(solution) == self.mask_minutes, (
                f"Solution length {len(solution)} mismatch expected length {self.mask_minutes}. Content of solution: {solution}"
            )
